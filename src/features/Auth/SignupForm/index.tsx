...
const SignupForm = (props: Props) => {
  ...
  const FormSchema = z
    .object({
      ...
      phone: z
        .string()
        .regex(phoneRegex, t("InvalidPhone"))
        .min(7, t("Min7"))
        .max(15, t("Max15")),
  ...
  const handleSubmit = async () => {
    ...
    let role: any = await roles?.find(
      (role: any) => role?.attributes?.role === form.getValues().role
    );
    ...
    try {
      ...
      if (responseData.error) {
        const message = responseData?.error?.message
          ? responseData?.error?.message.replace(t("Em"), t("Emi"))
          : t("Sum");
        toast({
          variant: "destructive",
          description: message,
      ...
      {route === 0 && (
        <div className="py-[20px] px-[20px] h-full flex flex-col items-center ">
          ...
          <div className="flex flex-col space-y-2 mb-10 w-full max-w-[388px]">
            <h1 className="font-bold text-[36px] leading-[45.36px] text-[#F5F7F8]">
              {t("Title")}
            </h1>
            <h1 className="font-normal text-[20px] leading-[20px] tracking-[1px] text-[#F5F7F8]  ">
              {t("SubTitle")}
            </h1>
          </div>
          ...
          <Form {...form}>
            ...
            >
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="relative">
                          <PureInput
                            type="email"
                            className="bg-black h-[48px]  placeholder:text-slate-400 text-[16px] leading-[24px] text-[#F5F7F8]"
                            {...field}
                          />
                          <div className="bg-black px-1 text-[12px] leading-[16px] font-normal absolute -top-[7px] left-[10px] text-[#F5F7F8]">
                            {t("Email")}
                          </div>
                        </div>
                      </FormControl>
              ...
              {/* the error message */}
              <h1 className="text-red-500 text-[14px] font-medium mt-[50px]">
                {error}
              </h1>
              {/* signup */}
              <div className=" w-full flex justify-center mt-[5px]">
                <button
                  type="submit"
                  className="py-[12px] z-10  w-full max-w-[388px]  cursor-pointer  bg-[#357EF8] flex justify-center rounded-[24px] gap-[12px]"
                >
                  <div className="flex space-x-2 items-center">
                    {loader ? (
                      <TbLoader3 className="text-white w-7 h-7 animate-spin" />
                    ) : (
                      <h1 className="text-[#F5F7F8] text-[16px] leading-[24px] ">
                        {t("Signup")}
                      </h1>
                    )}
                  </div>
          ...
          <div className="w-full max-w-[388px] my-[30px] flex space-x-3 items-center">
            ...
            <h1 className="font-medium text-[12px] leading-[18px] tracking-[0.15px] text-[#F5F7F8] whitespace-nowrap">
              {t("Continue")}
            </h1>
            <div className="w-[50%] h-[1px] bg-[#F5F7F8]" />
          </div>
          ...
          <div className=" w-full flex justify-center ">
            ...
            >
              <div className="flex space-x-2 items-center">
                <Image
                  src={"/onboard/Google.png"}
                  width={300}
                  height={300}
                  alt="Google icon"
                  className="h-[20px] w-[20px]"
                />
                <h1 className="text-[#1A1A1A] text-[16px] leading-[24px] ">
                  {t("Google")}
                </h1>
              </div>
            </div>
          ...
          <div className="flex justify-center mt-[40px]">
            <h1 className="font-semibold text-[16px] leading-[24px] tracking-[0.15px] text-[#F5F7F8]">
              {t("Account")}{" "}
              <span
                onClick={() => {
                  router.push("/sign-in");
                }}
                className="text-[#357EF8] cursor-pointer"
              >
                {t("SignIn")}
              </span>{" "}
            </h1>
