...
const EmailVerification = ({ setRoute, email }: Props) => {
    ...
    const handleResendClick = async () => {
        ...
        try {
            const responseData = await fetcher(
                `${process.env.NEXT_PUBLIC_STRAPI_URL}/send-email`,
                {
                    headers: { 'Content-Type': 'application/json' },
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className='py-[20px] px-[20px] h-full flex flex-col items-center '>
                ...
                <div className='w-full max-w-[388px] mt-[20px] mb-[50px] flex flex-col space-y-9' >
                    <div className='flex flex-col space-y-2'>
                        <h1 className='font-medium text-[24px] leading-[32.47px] text-[#FFFFFF]'>{t("EmailVeri")}</h1>
                ...
                <div className='mt-[30px] w-full max-w-[388px]'>
                    ...
                    {resendTimer === 0 ? (
                        <div className='flex justify-center'><button onClick={handleResendClick} className='text-center text-[16px]  font-medium leading-[28.16px] tracking-[0.2px] text-[#357EF8]'>Resend</button>
                        </div>
